...
# Get icons - تقريب الرقم لأقرب 5 عشان يطابق ملفات الأيقونات عندك
get_icon() {
    current=$(get_volume)
    if [[ "$current" == "Muted" ]]; then
        echo "$iDIR/muted-speaker.svg"
    else
        vol_raw=$(echo "$current" | sed 's/%//')
        rounded_vol=$(((vol_raw + 2) / 5 * 5))
        if [ "$rounded_vol" -gt 100 ]; then rounded_vol=100; fi
        echo "$iDIR/vol-${rounded_vol}.svg"
    fi
}

# Notify using swaync - إرسال التنبيه بالأيقونة والنص فقط بدون الشريط
notify_user() {
    if [[ "$(get_volume)" == "Muted" ]]; then
        notify-send -e -h string:x-canonical-private-synchronous:volume_notif -u low -i "$(get_icon)" "Volume: Muted"
    else
        volume_val=$(get_volume)
        # تم حذف int:value لإخفاء الشريط الأفقي
        notify-send -e -h string:x-canonical-private-synchronous:volume_notif -u low -i "$(get_icon)" "Volume: $volume_val"
    fi
}
...
notify_mic_user() {
    ...
    icon=$(get_mic_icon)
    notify-send -a -r 91190 -t 800 -i "$icon" "Mic Level: $volume"
}
